...
void cargarComentario(const string &date){
    cout<<"Cargando Comentarios..."<<endl;
    system("cls");

    mostrarEncabezadoTabla("COMENTARIOS EN " + date);
    ...
    if(contador == 0) cout << "No hay comentarios en " << date << "." << endl;
    else cout << "Total de comentarios encontrados: " << contador << endl;
...
void menorStock() {
    mostrarEncabezadoTabla("5 PRODUCTOS CON MENOR STOCK (Orden Ascendente)");
    mostrarColumnas(false);
    productosSortStock();
    for (int i = 0; i < 5; i++) {
        mostrarFilaProducto(Productos[i]);
    }
    mostrarLineaSeparadora();
    cout << "Mostrando los 5 productos con menor stock.\n\n";
}

void menorPrecio() {
    mostrarEncabezadoTabla("PRODUCTO CON MENOR PRECIO");

    auto it = min_element(Productos.begin(), Productos.end(),
        [](const Producto& a, const Producto& b) {
...
void mayorPrecio() {
    ...
    auto it = max_element(Productos.begin(), Productos.end(),
        [](const Producto& a, const Producto& b) {
            return a.precio < b.precio;
        }
    );
    ...
    for (int opcion=-1;;) {
        ...
        switch (opcion) {
            ...
            case 5: {
                ...
                cout <<"2. Comentarios de fecha" << endl;
                cout <<"3. Precio maximo y minimo" << endl;
                int subOpcion;
                entradaEnRango(subOpcion, 1, 3, "Seleccione una opcion (1-3): ", "Opcion invalida. Intente nuevamente.");
                system("cls");
                if (subOpcion == 1) menorStock();
                else if (subOpcion == 2) {
                	string date;
                	cout << "Ingrese la fecha (aaaa/mm/dd): ";
                	cin >> date;
                	cargarComentario(date);
				}
                else if (subOpcion == 3) {
					mayorPrecio(); 
					menorPrecio();
